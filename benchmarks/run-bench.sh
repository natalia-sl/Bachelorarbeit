...
NBP_KNOBS=(nbp_geometry nbp_pow_n nbp_rebal_k \
           nbp_prune nbp_epoch nbp_epoch_min \
           nbp_adaptive_bounds nbp_access nbp_th_every nbp_epoch_every)
...
# adaptive bounds: per-zone fault counters (all NA on older kernels)
ZONE_KEYS=(always if_free_ok if_free_no never warmup)
...
for rep in $(seq 1 "$REPS"); do
    ...
    ab_on=$(hist_val adaptive_bounds)
    always_end=$(hist_val always_ms); if_free_end=$(hist_val if_free_ms)
...

    {
      ...
      if [[ "$ab_on" == "1" ]]; then
        echo "--- adaptive bounds ---"
        echo "always_ms = $always_end   if_free_ms = $if_free_end   peak_bucket = $peak_end"
        IFS=, read -r n_always n_ifok n_ifno n_never n_warm <<<"$ZONE_DELTAS"
        echo "always (always promote)       = $n_always"
        echo "if_free_ok (space available)  = $n_ifok"
        echo "if_free_no (tier full)        = $n_ifno"
        echo "never (past the peak)         = $n_never"
        echo "warmup (pre-first-recompute)  = $n_warm"
        # if the middle zone never resolves either way it is not doing work
        if [[ "$n_ifok" =~ ^[0-9]+$ && "$n_ifno" =~ ^[0-9]+$ && $((n_ifok+n_ifno)) -gt 0 ]]; then
          echo "if_free promoted share        = $(( 100*n_ifok/(n_ifok+n_ifno) ))%"
        fi
        if [[ "$n_warm" =~ ^[0-9]+$ && "$n_always" =~ ^[0-9]+$ && $n_warm -gt $((n_always+1)) ]]; then
          echo "NOTE: warm-up faults exceed always - the run may be too short for the recompute interval"
        fi
      fi
