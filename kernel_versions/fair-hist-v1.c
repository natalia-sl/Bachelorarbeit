 ...
 * TIME-ONLY BUILD.
 *
 * Access-based epoch tracking is removed from this variant, not merely
 * defaulted off: nbp_access, nbp_th_every, nbp_epoch_every, the odometer
 * bookmarks and the whole access branch of nbp_hist_maybe_decay() are
 * gone. Every geometry knob (spacing 0-4, prune, epoch, pow_n, nbuckets,
 * rebal_k) is unchanged, so this is a single-variable control against the
 * access-mode build.
 *
 * Deleting rather than disabling is deliberate: a knob left in place can
 * be set by a stale script or a leftover shell, and we have already lost
 * a night to a kernel that was not the one we thought was booted.
 *
 * Total hint faults reaching the tiering path, INCLUDING pruned ones. In
 * this build it drives nothing -- it exists only as the denominator for
 * pruned_pct. It stays atomic_t so the fault path is byte-for-byte the
 * same cost as the access-mode build; making it a plain ++ here would
 * introduce a second difference and confound the comparison.
 */
...
static void nbp_hist_update_threshold(void)
{
	...
	for (b = 0; b < nbp_hist_nbuckets(); b++) {
		if (acc + nbp_hist[b] >= budget)
			break;
		acc += nbp_hist[b];
	}
	if (b >= nbp_hist_nbuckets()) {
...
static void nbp_hist_rebalance(void)
{
	...
	for (k = 1; k < n; k++) {
		...
		while (b < n && acc + nbp_hist[b] < target) {
			acc += nbp_hist[b];
			b++;
		}
		if (b >= n) {
...
static int nbp_hist_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "tracking = time-only (no access mode in this build)\n");
	for (b = 0; b < nbp_hist_nbuckets(); b++)
