...
#if 0	/* stock servo loop, superseded by the NBP histogram threshold */
...
static unsigned long nbp_pruned;	/* faults discarded: latency >= cutoff */

/*
 * Pruning cutoff: faults at or above this latency are not considered at
 * all -- neither counted into the histogram nor promoted. 1000 ms equals
 * the default sysctl_numa_balancing_scan_period_min, i.e. the hint-fault
 * latency floor invariant, so this is a derived constant, not a new knob.
 */
#define NBP_PRUNE_MS	1000
 ...
 *   0 = fractional log2, 16 buckets, floor 512 ms   (pre-pruning control)
 ...
 *
 * Table-driven on purpose: a closed-form inverse would need a different
 * integer root per n, whereas a boundary table is one code path for all of
 * them -- at most 9 branch-predictable compares on the insert path.
 */
#define NBP_POW_VARIANTS	4
static const unsigned short nbp_pow_lower[NBP_POW_VARIANTS][10] = {
	/* n = 1.5 */ { 0, 32, 89, 164, 253, 354, 465, 586, 716, 854 },
	/* n = 2.0 */ { 0, 10, 40,  90, 160, 250, 360, 490, 640, 810 },
	/* n = 2.5 */ { 0,  3, 18,  49, 101, 177, 279, 410, 572, 768 },
	/* n = 3.0 */ { 0,  1,  8,  27,  64, 125, 216, 343, 512, 729 },
};
static unsigned int nbp_pow_n = 1;	/* index: 0=1.5 1=2.0 2=2.5 3=3.0 */
 ...
 * that each bucket holds ~1/N of the observed faults. For a fixed bucket
 * count that minimises worst-case quantile error, because the bucket
 * containing the target percentile holds at most 1/N of the mass.
 *
 * Chicken-and-egg: the new boundaries are estimated from the OLD histogram
 * by the same interpolation used for the threshold, so the first estimate
 * inherits the linear-density assumption we are trying to escape. It is
 * self-correcting rather than exact -- wherever a bucket is wide AND full
 * the next rebalance splits it, so the assumption is applied to
 * progressively narrower intervals and its error shrinks each epoch.
 *
 * Hard limit worth knowing: latency arrives pre-quantized by
 * folio_xchg_access_time(). If a single discrete latency value carries
 * more than 1/N of the mass, NO boundary placement can equalise the
 * buckets, and this scheme will converge to something visibly unequal.
 * That is a property of the input, not a failure of the algorithm.
 *
 * Concurrency: double-buffered. Readers take the published index once and
 * scan one coherent array; the rebalancer fills the inactive buffer and
 ...
 * where the linear-density interpolation supplies the entire answer). A
 * binary threshold makes that one fragile number decide everything. Here
...
{
	switch (nbp_spacing) {
	case 0:	 return NBP_HIST_BUCKETS;
 ...
 *   0 = control: halve all counts every scan_period_max (~60 s)
 *   1 = flush:   every NBP_EPOCH_MS close the epoch; if it saw at least
 *       nbp_epoch_min samples, zero the histogram (fresh window); on a
 *       quiet epoch keep the data but age it one bucket colder instead.
 * The threshold survives a flush (empty histogram leaves it unchanged)
 * until ~1 s of new data exists.
 */
#define NBP_EPOCH_MS	5000
static unsigned int nbp_epoch = 1;
static unsigned int nbp_epoch_min = 100;
static unsigned int nbp_epoch_samples;	/* inserts since last epoch close */
static unsigned long nbp_flushes;	/* epochs closed by flushing */
static unsigned long nbp_shifts;	/* epochs closed by cold-shift aging */

/*
 * Fault-driven ("access-based") maintenance cadence, nbp_access = 1.
 *
 * Instead of firing on the wall clock, threshold recompute and epoch flush
 * fire after a fixed number of hint faults. Naming caveat worth carrying
 * into the writeup: this counts hint FAULTS, not accesses. NUMA balancing
 * marks PTEs PROT_NONE periodically and restores them on the first touch,
 * so a page touched a million times between two scans produces exactly one
 * fault. The odometer therefore measures distinct hot pages touched per
 * scan period -- a sampled working-set signal, not an access count. That
 * is the only access proxy available without hardware sampling, which this
 * design excludes by construction.
 *
 * Cadence calibration: at the ~35k faults/s measured on pr, 4096 faults is
 * ~0.12 s against scan_period_min's ~1 s, so at these defaults access mode
 * adapts roughly 8x faster than time mode. A time-vs-access comparison at
 * these values therefore varies two things at once (what drives the clock,
 * and how fast it ticks). To isolate the first, set the intervals to the
 * observed fault rate times the time-mode periods -- for pr that is about
 * nbp_th_every = 32768 and nbp_epoch_every = 175000. Left at the original
 * values by default so earlier runs stay comparable.
 */
static unsigned int nbp_access;		/* 0 = time-based (default) */
static unsigned int nbp_th_every = 4096;
static unsigned int nbp_epoch_every = 65536;

/*
 * Total hint faults reaching the tiering path, INCLUDING pruned ones.
 * Pruning is a promotion decision, not a statement that no memory activity
 * occurred: ticking only on survivors made the epoch clock run at the
 * survival rate, so at a 96% prune rate a 4096-fault epoch needed ~110k
 * real faults and the threshold went stale exactly when pruning was
 * heaviest.
 *
 * Serves two roles: denominator for pruned_pct in every build, and the
 * epoch clock when nbp_access = 1. atomic_t because of the second role --
 * a plain ++ loses updates across CPUs, which would make epoch length
 * depend on core count. The histogram buckets stay non-atomic; they are
 * sampled statistics that tolerate drift, whereas a clock does not.
 *
 * Read as unsigned int so that cnt - mark stays correct across the 32-bit
 * wrap (atomic_t is signed and goes negative at 2^31).
 */
static atomic_t nbp_faults = ATOMIC_INIT(0);
...
static unsigned int nbp_hist_bucket(unsigned int latency)
{
	...
	switch (nbp_spacing) {
	case 1:				/* static 10 x 100 ms */
		return min(latency / 100, 9u);
	case 2:				/* log2: 0,1,2,4,...,256+, tail in 9 */
		return min((unsigned int)fls(latency), 9u);
	...
	v = latency >> NBP_HIST_FLOOR_SHIFT;	/* fractional log2 */
	if (v < 4)
		return v;
	msb = fls(v) - 1;
	return min(((msb - 2) << 2) + 4 + ((v >> (msb - 2)) & 0x3),
		   (unsigned int)NBP_HIST_BUCKETS - 1);
}
...
static unsigned int nbp_hist_bucket_lower(int b)
{
	...
	if (b < 4)
		...
		lo = (4 + (b & 0x3)) << ((b - 4) >> 2);
	return lo << NBP_HIST_FLOOR_SHIFT;
}
...
static unsigned int nbp_hist_bucket_width(int b)
{
	...
	case 2:
		if (b == 0)
			return 1;
		if (b == 9)
			return NBP_PRUNE_MS - 256;
		return 1u << (b - 1);
	case 3: {
		const unsigned short *lo = nbp_pow_lower[nbp_pow_n];

		/* the range closes at the prune cutoff by construction */
		return (b == 9 ? NBP_PRUNE_MS : lo[b + 1]) - lo[b];
	}
	case 4: {
		const unsigned short *e = nbp_edges[READ_ONCE(nbp_edges_idx)];
	...
	if (b < 4)
		return 1 << NBP_HIST_FLOOR_SHIFT;
	return (1u << ((b - 4) >> 2)) << NBP_HIST_FLOOR_SHIFT;
}

/*
 * Walk from the hot end until NBP_TARGET_PCT of mass is covered, then
 * interpolate inside the boundary bucket for a continuous threshold in ms.
 * Runs only on the periodic path, never per fault.
 */
static void nbp_hist_update_threshold(void)
{
	...
	/*
	 * Snapshot before doing anything else.
	 *
	 * nbp_hist[] is incremented by every CPU on the fault path and
	 * zeroed wholesale by the epoch flush. The cmpxchg guards serialise
	 * flush-against-flush and recompute-against-recompute, but they are
	 * DIFFERENT cmpxchgs: one CPU can be walking the histogram here
	 * while another zeroes it in the flush branch.
	 *
	 * Reading a bucket twice -- once to test it in the walk, once as the
	 * divisor below -- therefore allowed a nonzero bucket to become 0
	 * between the two reads, giving an integer divide-by-zero. That is a
	 * #DE, and this function runs from do_numa_page() with the PTE
	 * spinlock held, so the resulting oops leaks that lock: the dying
	 * task spins forever in exit_mmap()->__pte_offset_map_lock(), its
	 * CPU never schedules or reaches a quiescent state, and the box
	 * takes down lru_add_drain_all() and synchronize_rcu() with it.
	 *
	 * Working from a private copy makes the walk and the division see
	 * one consistent histogram. It costs 64 bytes of stack and removes
	...
	budget = total * NBP_TARGET_PCT / 100;
	if (!budget)
		budget = 1;
	...
	need = budget - acc;
	/*
	 * snap[b] > 0 is guaranteed: the walk only breaks when
	 * acc + snap[b] >= budget while acc < budget. Belt and braces
	 * anyway -- a divide-by-zero here costs the machine, not the run.
	 */
	if (!snap[b])
		return;

	th_ms = nbp_hist_bucket_lower(b) +
		(unsigned int)((unsigned long)nbp_hist_bucket_width(b) *
			       need / snap[b]);
	/* never round to "promote nothing" when bucket 0 dominates */
	th_ms = max(th_ms, 1U);
	 ...
	 */
	{
		...
		for (b = 0; b < n; b++) {
			...
			if (!wb)
				continue;
			/* snap[b]/wb > (5/4) * snap[peak]/wp  */
			if ((unsigned long)snap[b] * wp * NBP_PEAK_HYST_DEN >
			    dp * wb * NBP_PEAK_HYST_NUM) {
				peak = b;
				wp = wb;
				dp = (unsigned long)snap[b];
			}
		}
...
/*
 * Periodic maintenance on two cadences borrowed from existing sysctls:
 * threshold recompute every scan_period_min (~1 s) so the boundary tracks
 * the workload; decay-halving every scan_period_max (~60 s) so history
 ...
 * better first estimate. Both buffers are seeded so the first publish is
 * coherent whichever way the index lands.
 */
static void nbp_hist_seed_edges(void)
{
	...
	WRITE_ONCE(nbp_edges_idx, 0);
}

/*
 * Recompute boundaries so each bucket holds ~1/N of the mass. Must run
 * BEFORE the histogram is zeroed -- it reads the epoch that is ending.
 */
static void nbp_hist_rebalance(void)
{
	 ...
	 * write 0 to it -- which would divide by zero below, in a function
	 * reached from do_numa_page() with the PTE spinlock held. That is
	 * the identical failure mode as the histogram-flush race: a #DE in
	 * atomic context leaks the lock and wedges the machine. Clamping
	 * the upper end too, since src[k] * (kd - 1) would otherwise
	 * overflow for large kd. Reading once also makes the loop immune to
	 * a concurrent write landing between iterations.
	 */
	const unsigned int kd = clamp(READ_ONCE(nbp_rebal_k), 1u, 64u);

	/* same torn-read hazard as nbp_hist_update_threshold(): snapshot */
	for (b = 0; b < n; b++) {
		snap[b] = READ_ONCE(nbp_hist[b]);
		total += snap[b];
	}
	...
	/*
	 * Too sparse to trust: re-estimating from noise would jitter the
	 * boundaries, and every boundary move perturbs the threshold, which
	 * perturbs promotion, which perturbs the distribution. Keep the old
	 * geometry rather than feed that loop.
	 */
	if (total < (unsigned long)n * NBP_REBAL_MIN_PER_BUCKET) {
		nbp_rebal_skips++;
		return;
	}
	...
	b = 0;
	for (k = 1; k < n; k++) {
		unsigned long target = total * k / n;
		unsigned int est, damped;

		/* monotone walk: b and acc carry across iterations */
		while (b < n && acc + snap[b] < target) {
			acc += snap[b];
			b++;
		}
		if (b >= n) {
			...
			unsigned int w = nbp_hist_bucket_width(b);
			unsigned int rem = (unsigned int)(target - acc);

			est = nbp_hist_bucket_lower(b);
			if (snap[b])
				est += (unsigned int)((unsigned long)w * rem /
						      snap[b]);
		}

		/* damp: the boundaries are a control input, not a measurement */
		damped = (src[k] * (kd - 1) + est) / kd;

		/*
		 * Strict monotonicity, with headroom for the edges still to
		 * be placed. Clamping every saturated edge to NBP_PRUNE_MS-1
		 * would let two adjacent edges land on the same value, and a
		 * zero-width bucket makes the interpolation in
		 * nbp_hist_update_threshold() meaningless. Reserving one ms
		 * per remaining edge keeps every bucket at least 1 ms wide.
		 * Always satisfiable: NBP_PRUNE_MS (1000) >> NBP_HIST_BUCKETS.
		 */
		if (damped <= dst[k - 1])
			damped = dst[k - 1] + 1;
		if (damped > NBP_PRUNE_MS - n + k)
...
static void nbp_hist_maybe_decay(void)
{
	...
	if (nbp_access) {
		unsigned int cnt = (unsigned int)atomic_read(&nbp_faults);
		unsigned int mark;

		/*
		 * READ_ONCE: the marks are written by other CPUs via cmpxchg,
		 * so the compiler must not reload them between the compare
		 * and the swap.
		 *
		 * cmpxchg rather than a modulo test makes the work happen
		 * exactly once: several CPUs may observe the condition, only
		 * the one that wins the swap acts. Unsigned subtraction
		 * rather than cnt >= mark + interval so wraparound is safe.
		 *
		 * The threshold check comes first on purpose. When both fire
		 * in the same call the recompute then sees the full epoch,
		 * and the flush's own recompute lands on a zeroed histogram
		 * and returns early, leaving that good value published.
		 */
		mark = READ_ONCE(nbp_access_th_mark);
		if (cnt - mark >= nbp_th_every &&
		    cmpxchg(&nbp_access_th_mark, mark, cnt) == mark)
			nbp_hist_update_threshold();
		...
		/*
		 * Return before the wall-clock paths: nbp_epoch, the control
		 * halving mode, nbp_epoch_min and shift-aging are all
		 * inactive here. The quiet-epoch exception is not handled but
		 * dissolved -- no faults means the odometer does not advance,
		 * so the epoch simply stretches in wall time.
		 */
		return;
	}

	start = nbp_hist_th_start;
	if (now - start > sysctl_numa_balancing_scan_period_min &&
	    cmpxchg(&nbp_hist_th_start, start, now) == start)
		nbp_hist_update_threshold();

	start = nbp_hist_start;
	if (now - start > (nbp_epoch ? NBP_EPOCH_MS :
		...
		if (!nbp_epoch) {		/* control: gradual halving */
			...
			for (b = 0; b < n; b++)	/* flush: fresh window */
				nbp_hist[b] = 0;
			nbp_flushes++;
		} else {			/* quiet: age one bucket colder */
			nbp_hist[n - 1] += nbp_hist[n - 2];
			for (b = n - 2; b > 0; b--)
				nbp_hist[b] = nbp_hist[b - 1];
			nbp_hist[0] = 0;
			nbp_shifts++;
		}
		nbp_epoch_samples = 0;
				...
				int src_nid, int dst_cpu)
{
	...
	if (folio_use_access_time(folio)) {
		...
		atomic_inc(&nbp_faults);

		if (nbp_prune && latency >= NBP_PRUNE_MS) {
			nbp_pruned++;
			return false;	/* pruned: not binned, not promoted */
		}

		nbp_hist[nbp_hist_bucket(latency)]++;
		nbp_epoch_samples++;
		...
		/*
		 * Histogram accepted the page. The stock rate limiter stays
		 * as a safety cap; nbp_rl_rejected counts how often it
		 * overrides us -- it must stay 0 for the evaluation claim
		 * that promote_rate_limit never governs behavior.
		 */
		rate_limit = sysctl_numa_balancing_promote_rate_limit <<
			(20 - PAGE_SHIFT);
		if (numa_promotion_rate_limit(pgdat, rate_limit,
					      folio_nr_pages(folio))) {
			nbp_rl_rejected += folio_nr_pages(folio);
			return false;
		}
		return true;
	}
...
#ifdef CONFIG_NUMA_BALANCING
static int nbp_hist_show(struct seq_file *m, void *v)
{
	int b;
	...
	seq_printf(m, "rl_rejected = %lu\n", nbp_rl_rejected);
	{	/* pruned fraction of all faults, in tenths of a percent */
		unsigned long faults = (unsigned int)atomic_read(&nbp_faults);
		unsigned long permille = faults ? nbp_pruned * 1000 / faults : 0;

		seq_printf(m, "pruned = %lu  faults = %lu  pruned_pct = %lu.%lu\n",
			   nbp_pruned, faults, permille / 10, permille % 10);
	}
	seq_printf(m, "flushes = %lu  shifts = %lu\n", nbp_flushes, nbp_shifts);
	/*
...
/*
 * Any change of bucket geometry invalidates the accumulated counts, so
 * both geometry knobs drop the histogram, the threshold and the counters.
 */
static void nbp_hist_reset(void)
{
	unsigned int now = jiffies_to_msecs(jiffies);
	int b;
	...
	WRITE_ONCE(nbp_hist_threshold_ms, 0);
	/*
	 * Rearm the wall-clock epochs too. Without this, a reset taken while
	 * running access-based leaves these timestamps arbitrarily stale, and
	 * the first fault after switching back to time-based sees a huge
	 * elapsed interval and fires a spurious decay on an empty histogram.
	 */
	nbp_hist_start = now;
	nbp_hist_th_start = now;
	nbp_pruned = 0;
	nbp_rl_rejected = 0;
	nbp_epoch_samples = 0;
	nbp_flushes = 0;
...
DEFINE_DEBUGFS_ATTRIBUTE(nbp_pow_n_fops, nbp_pow_n_get,
			 nbp_pow_n_set, "%llu\n");
...
late_initcall(nbp_hist_debugfs_init);
#endif
