...
static unsigned int nbp_hist_threshold_ms = UINT_MAX;
...
{
	return nbp_spacing ? 10 : NBP_HIST_BUCKETS;
}
...
/*
 * Access-based tracking (/sys/kernel/debug/nbp_access = 1): maintenance
 * cadence counted in histogram inserts instead of wall time. Threshold
 * recompute every nbp_th_every inserts, full flush every nbp_epoch_every
 * inserts. No quiet-epoch special case exists in this mode: low traffic
 * simply stretches the epoch in wall time until enough samples arrive.
 */
...
static unsigned int nbp_access_count;	/* inserts, wraps harmlessly */
static unsigned int nbp_access_th_mark;
static unsigned int nbp_access_ep_mark;
...
static void nbp_hist_update_threshold(void)
{
	...
	if (b >= nbp_hist_nbuckets()) {
		WRITE_ONCE(nbp_hist_threshold_ms, UINT_MAX);
		return;
	}
				...
				int src_nid, int dst_cpu)
{
	...
	if (folio_use_access_time(folio)) {
		...
		if (nbp_prune && latency >= NBP_PRUNE_MS) {
			nbp_pruned++;
			return false;	/* pruned: not counted, not promoted */
		}
...
static int nbp_hist_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "access = %u  access_count = %u\n",
		   nbp_access, nbp_access_count);
	for (b = 0; b < nbp_hist_nbuckets(); b++)
